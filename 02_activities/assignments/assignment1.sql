...
SELECT vendor_id, count(*) AS rented_count
...
SELECT customer.*
FROM (
	SELECT customer_id,  sum(cost_to_customer_per_qty*quantity) AS amount_spent
	FROM customer_purchases
	GROUP BY customer_id
	HAVING sum(cost_to_customer_per_qty*quantity) > 2000) customer_bumper INNER JOIN customer on customer_bumper.customer_id = customer.customer_id
order by customer_last_name, customer_first_name
